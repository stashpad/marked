...
var Renderer = /*#__PURE__*/function () {
  function Renderer(options) {
    console.log('setting', _extends({}, exports.defaults, options));
    this.options = _extends({}, exports.defaults, options);
  }

  var _proto = Renderer.prototype;

  _proto.generateDataAttributes = function generateDataAttributes(origin) {
  ...
  _proto.code = function code(_code, infostring, escaped, origin) {
    ...
    if (!lang) {
      return "<pre" + this.generateDataAttributes(origin) + "><code>" + (escaped ? _code : escape(_code, true)) + '</code></pre>\n';
    }

    return "<pre" + this.generateDataAttributes(origin) + "><code class=\"" + this.options.langPrefix + escape(lang, true) + '">' + (escaped ? _code : escape(_code, true)) + '</code></pre>\n';
  }
  /**
   * @param {string} quote
   */
  ...
  _proto.blockquote = function blockquote(quote, origin) {
  ...
  _proto.heading = function heading(text, level, raw, slugger, origin) {
    // is header from 2 lines or 1
    var expectedHashLength = Array(level).fill('#').join('') + ' ' + text;
    var expectedLength = expectedHashLength.length;
    var actualLength = origin.end - origin.start - '\n'.length;
    var hashHeader = true;
  ...
  _proto.listitem = function listitem(text, task, checked, origin) {
    return "<li" + this.generateDataAttributes(origin) + ">" + text + "</li>\n";
  };

  _proto.checkbox = function checkbox(checked, origin) {
    console.log('received', origin);
    return "<input" + this.generateDataAttributes(origin) + " " + (checked ? 'checked="" ' : '') + 'disabled="" type="checkbox"' + (this.options.xhtml ? ' /' : '') + '> ';
  }
  /**
   * @param {string} text
   */
  ...
  _proto.paragraph = function paragraph(text, origin) {
  ...
  _proto.strong = function strong(text, origin) {
  ...
  _proto.em = function em(text, origin) {
  ...
  _proto.codespan = function codespan(text, origin) {
  ...
  _proto.del = function del(text, origin) {
...
var Parser = /*#__PURE__*/function () {
  ...
  _proto.parse = function parse(tokens, top) {
    ...
    var out = '',
        ...
        checkbox,
        ret;
    var l = tokens.length; // console.log('all tokens', tokens);
    ...
    for (i = 0; i < l; i++) {
      ...
      switch (token.type) {
        ...
        case 'heading':
          {
            out += this.renderer.heading(this.parseInline(token.tokens), token.depth, unescape(this.parseInline(token.tokens, this.textRenderer)), this.slugger, token.origin);
            continue;
          }

        case 'code':
          {
            out += this.renderer.code(token.text, token.lang, token.escaped, token.origin);
            continue;
          }
